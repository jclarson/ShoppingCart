...
const Products = (props) => {
  ...
  console.log(`Rendering Products ${JSON.stringify(items)}`);
  ...
  const deleteCartItem = (index) => {
    let name = cart.filter((item, i) => index == i)[0].name
    let updatedItems = items.map(item => {
    ...
    return (
      <li key={index}>
        ...
        <Button variant="primary" size="large">
          {item.name}: ${item.cost}<br />In Stock: {item.instock}
        </Button>
        <input name={item.name} type="submit" onClick={addToCart} value="Add to Cart"></input>
      </li>
    );
  });
//  console.log(`cart is now ${JSON.stringify(cart)}`)
...
//console.log(cartList)
  ...
  const restockProducts = (url) => {
    ...
    let newData = data.map(item => {
      ...
      ndStrings.push(`${item.name}~${item.country}~${item.cost}`);
      return { name, country, cost, instock };
    });
    
    let iStrings = [];
    let restockedItems = items.map(item => {
      iStrings.push(`${item.name}~${item.country}~${item.cost}`);
